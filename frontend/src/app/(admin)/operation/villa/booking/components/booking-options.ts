...
import type { BusinessScopedOption } from "./booking-form"

export interface BookingOptions {
  /** Villa businesses (type 2) for the FK select. */
...
/**
 * Load the villa + room options the booking form needs (server-side). Rooms are
 * only returned by the business detail endpoint, so each villa is fetched once
 * and its rooms flattened into a single business-tagged list.
 */
export async function loadBookingOptions(): Promise<BookingOptions> {
