...
export interface CropColumnHandlers {
  onEdit?: (crop: Crop) => void
  onDelete?: (crop: Crop) => void
}

/**
 * Column factory for the Crop list. Handlers are injected by the screen so the
 * column array can be memoised with a stable identity.
 */
export function getCropColumns({
  onEdit,
  onDelete,
}: CropColumnHandlers = {}): ColumnDef<Crop>[] {
  return [
    {
      accessorKey: "name",
      header: ({ column }) => (
